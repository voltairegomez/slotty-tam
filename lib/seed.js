...
async function migrate() {
  const adds = [
    ...
    'ALTER TABLE merch ADD COLUMN image_url TEXT',
  ];
...
export async function insertContent(data) {
  ...
  for (const m of data.merch) batch.push({ sql: `INSERT INTO merch (name,price,emoji,badge,thumb_class,sold_count,image_url) VALUES (?,?,?,?,?,?,?)`, args: [m.name, m.price, m.emoji, m.badge ?? null, m.thumb_class || 'm1', m.sold_count || 0, m.image_url ?? null] });
